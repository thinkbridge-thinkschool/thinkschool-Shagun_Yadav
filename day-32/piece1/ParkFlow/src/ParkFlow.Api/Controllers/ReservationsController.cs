...
public sealed class ReservationsController(ReservationApplicationService reservations) : ControllerBase
{
    ...
    {
        var summaries = await reservations.GetMineAsync(GetCurrentUserId(), cancellationToken);
        return Ok(summaries);
    }
    ...
    {
        var result = await reservations.ConfirmAsync(id, GetCurrentUserId(), cancellationToken);
