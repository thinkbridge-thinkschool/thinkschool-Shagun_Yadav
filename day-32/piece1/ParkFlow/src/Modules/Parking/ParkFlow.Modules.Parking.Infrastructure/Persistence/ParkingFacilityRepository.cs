...
public sealed class ParkingFacilityRepository(ParkingDbContext dbContext) : IParkingFacilityRepository
{
    ...
    public async Task<IReadOnlyList<Domain>> GetAllAsync(CancellationToken cancellationToken = default) =>
        await dbContext.Facilities.ToListAsync(cancellationToken);
