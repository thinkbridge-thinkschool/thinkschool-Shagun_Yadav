...
public sealed class ParkingSpotRepository(ParkingDbContext dbContext) : IParkingSpotRepository
{
    ...
    public async Task<IReadOnlyList<Domain>> GetByFacilityIdAsync(Guid facilityId, CancellationToken cancellationToken = default) =>
        await dbContext.Spots
            .Where(s => s.FacilityId == facilityId)
            .OrderBy(s => s.FloorLevel).ThenBy(s => s.SpotNumber)
