...
{
    /// <summary>Day 32: read-only spot grid for the frontend — never mutates state.</summary>
    public async Task<IReadOnlyList<ParkingSpotSummary>> ListByFacilityAsync(Guid facilityId, CancellationToken cancellationToken = default)
    {
        var spots = await spotRepository.GetByFacilityIdAsync(facilityId, cancellationToken);

        return spots
            .Select(s => new ParkingSpotSummary(s.Id, s.FacilityId, s.FloorLevel, s.SpotNumber, s.SpotType.ToString(), s.Status.ToString()))
            .ToList();
    }
